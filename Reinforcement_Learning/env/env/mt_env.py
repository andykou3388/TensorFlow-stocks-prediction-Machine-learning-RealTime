...
from env.simulator.mt5 import MtSimulator, OrderType
...
class MtEnv(gym.Env):
    ...
    def __init__(
            ...
            window_size: int, log_dir, time_points: Optional[List[datetime]]=None,
            ...
            render_mode=None, normalise=None, early_stop_ratio=0.1,
        ) -> None:
        ...
        self.prices, self.signal_features, self._start_tick, self._end_tick = self._process_data()
        ...
        self.observation_space = spaces.Dict({
            ...
            'bal_diff': spaces.Box(low=-INF, high=INF, shape=(window_size,), dtype=np.float64),
    ...
    def reset(self, seed=None, options=None) -> Dict[str, np.ndarray]:
        ...
        self.bal_diff = self.window_size * [0]
        info = self._create_info()
        self.history = [info]
        return self._get_observation(), info

    def total_steps(self): return self._end_tick - self._start_tick

    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict[str, Any]]:
        # Get Last Balance
        last_balance = self.simulator.balance
        ...
        # Early Stop (out of balance)
        if self.early_stop_ratio is not None:
            if (self.simulator.balance < (self.simulator.init_balance * self.early_stop_ratio)) and (self.simulator.equity < (self.simulator.init_balance * self.early_stop_ratio)):
                print(f'Early Stopout (Reason: Balance too low [{round(self.simulator.balance, 2)}/{self.simulator.init_balance}]) at {self._current_tick - self._start_tick}/{self.total_steps()}')
        ...
        # Write history to the Summary
        with self.writer.as_default():
            tf.summary.scalar(f'balance/Balance-{self.episode}', self.simulator.balance, step=(self._current_tick - self._start_tick))        

        info = self._create_info(
            orders=orders_info, closed_orders=closed_orders_info, step_reward=step_reward,
        )
        self.bal_diff.append(self.simulator.balance-last_balance)
    ...
    def _process_data(self) -> np.ndarray:
        ...
        for symbol in self.trading_symbols:
            get_price_at = lambda time: self.original_simulator.price_at(symbol, time)[['Open', 'High', 'Low', 'Close']]
            ...
            prices[symbol] = pd.concat(p, axis=1).T

        # Extract Signal Features (will dropout early bars)
        # Get the starting and ending tick
        last_starting = None
        earliest_ending = None
        features = {}
        for pair, ohlc_data in prices.items():
            print(f'Extracting Feature of {pair}...')
            df = ohlc_data.rename(columns={'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close'})
            df = extract_features(df, self.normalise, normalise_path=f'data/scaler_{pair}_mt5.pkl')

            # Get the starting and ending tick
            df_start_time = df.index[0].to_pydatetime()
            df_end_time = df.index[-1].to_pydatetime()
            if last_starting is None: last_starting = df_start_time
            elif df_start_time > last_starting: last_starting = df_start_time
            if earliest_ending is None: earliest_ending = df_end_time
            elif df_end_time < earliest_ending: earliest_ending = df_end_time

            print(f'{pair} Starting Time: {df_start_time}')
            print(f'{pair} Ending   Time: {df_end_time}')
            print(f'{pair} Feature Shape: {df.values.shape}')
            features[pair] = df.values

        print(f'Env Start Time: {last_starting}')
        print(f'Env End   Time: {earliest_ending}')

        start_tick = self.time_points.index(last_starting) + self.window_size - 1
        end_tick = self.time_points.index(earliest_ending)
        print(f'Total    Tick: {len(self.time_points)}')
        print(f'Starting Tick: {start_tick}')
        print(f'Ending   Tick: {end_tick}')
        print(f'Env.     Tick: {end_tick - start_tick}')

        # Signal Feature Shape = (Bar Size, Feature Size * No. of Symbols)
        signal_features = np.column_stack(list(features.values()))

        # Add back null features to dropped early rows
        dummy = np.zeros((len(self.time_points) - signal_features.shape[0], signal_features.shape[1]))
        signal_features = np.concatenate((dummy, signal_features), axis=0)

        print(f'Env Feature Shape: {signal_features.shape}')
        assert signal_features.shape[0] == len(self.time_points)

        return prices, signal_features, start_tick, end_tick
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        observation = {
            ...
            'bal_diff': np.array(self.bal_diff[-self.window_size:]),
    ...
    def _render_simple_figure(
        ...
        for j, symbol in enumerate(self.trading_symbols):
            close_price = self.prices[symbol]['Close']
