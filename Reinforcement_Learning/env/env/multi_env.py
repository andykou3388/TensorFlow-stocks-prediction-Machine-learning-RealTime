...
def transform(position: Positions, action: int):
    if action == Actions.SELL:
        if position == Positions.LONG: return Positions.SHORT, True
        else: return Positions.SHORT, False

    elif action == Actions.BUY:
        if position == Positions.SHORT: return Positions.SHORT, True
        else: return Positions.LONG, False
...
class MultiTradeEnv(gym.Env):
    ...
    def __init__(self, data: Dict[str, pd.DataFrame], config: EnvConfig, log_dir, render_mode=None, clear_trade=True, flatten=False, normalise=None):
    ...
    def reset(self, seed=None, options=None):
        ...
        for symbol in self.symbol_names:
            self.memory[symbol] = {
                ...
                'position_history': (self.window_size + 1) * [Positions.SHORT.value],
            }
    ...
    def step(self, action: np.ndarray):
        ...
        for i, symbol in enumerate(self.symbol_names):
            ...
            # Get the existing position's returns array 
            step_returns.append(self.memory[symbol]['returns_history'][-1])
            ...
            if trades.empty == False:
                ...
                if profit > self._best_score:
                    if hasattr(self, '_model') == True:
                        ...
                        self._model.save(f'output/best_multi.zip')
                        
                    # Save the trades
                    self._best_score = profit
                    trades.to_csv(f'logs/multi_trades_train.csv', index=False)
