...
# result_files = glob.glob('d_result/sent_predi_MULTI_real_time_*.csv')
result_files = glob.glob('d_result/sent_predi_MULTI_real_time_2025_08_01.csv')
...
def read_single_file(filepath):
    try:
        # Read the file with pandas, handling the tab-separated format
        # We'll manually specify the column names since the header row is malformed
        column_names = [
            'index', 'Date', 'ticker', 'buy_sell_point', 'Close', 'Volume',
            'POS_score', 'POS_num', 'NEG_score', 'NEG_num', 'POS_score_list',
            'NEG_score_list', 'POS_models_name', 'NEG_moldel_name'
        ]
        
        df = pd.read_csv(filepath, sep='\t', names=column_names, header=0, on_bad_lines='skip')
