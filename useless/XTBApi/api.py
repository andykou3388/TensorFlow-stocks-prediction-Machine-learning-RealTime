...
# logging.config.fileConfig(r"C:\Users\Luis\Desktop\LecTrade\LecTrade\LogRoot\logging.conf")
...
class Client(BaseClient):
    ...
    def check_if_market_open(self, list_of_symbols):
        ...
        for symbol in response:
            ...
            if not today_values:
                ...
                continue
            if today_values[0]['fromT'] <= actual_tmsp <= today_values[0]['toT']:
    ...
    def open_trade(self, mode, symbol, dolars =0, custom_Messege ="", tp_per = 0.05, sl_per= 0.05, order_margin_per = 0, expiration_stamp = 0, comment = ""):
        ...
        elif mode_name == MODES.SELL_LIMIT.name:
            price = round(price * (1 - order_margin_per), 2)  # No mas de 3 decimales
        ...
        response = self.trade_transaction(symbol, mode, trans_type = 0,volume = volumen, price=price,
                                          customComment=custom_Messege,tp=tp, sl=sl, expiration = expiration_stamp, comment=comment  )
        ...
        if status_messg == 'Invalid prices(limit)':
            response = self.trade_transaction(symbol, mode, trans_type=0, volume=volumen, price=price_2,customComment=custom_Messege, tp=tp, sl=sl, expiration=expiration_stamp,comment=custom_Messege )
            status, status_messg = self.manage_response(expiration_stamp, response)
            price = price_2
        if status_messg == 'Invalid s/l or t/p price':
            sl, tp = self.get_tp_sl(mode, price, sl_per+ 0.012, tp_per+ 0.012)
            response = self.trade_transaction(symbol, mode, trans_type=0, volume=volumen, price=price,customComment=custom_Messege, tp=tp, sl=sl, expiration=expiration_stamp,comment=custom_Messege )
            status, status_messg = self.manage_response(expiration_stamp, response)
        ...
        if status != 3:
            ...
            raise TransactionRejected(str(status) + " Message : "+ status_messg + ' Stock '+symbol+ " Volumne: "+str(volumen))
        else:
    ...
    # for trade_Id, data_xtb in trades.items():
    def update_trade(self, order_id, data_xtb, tp_new_value , sl_new_value, expiration_stamp = 0):
        """open trade transaction"""
        self.LOGGER.debug(f"TRANS_TYPES.MODIFY. opening trade of {data_xtb.symbol}  with {data_xtb.mode} . Num_order: {data_xtb.order_id}")

        order_id = data_xtb.order_id
        symbol = data_xtb.symbol
        mode = data_xtb._trans_dict['cmd']
        trans_type = TRANS_TYPES.MODIFY.value
        volumen = data_xtb.volume
        price = data_xtb.price
        comment = data_xtb._trans_dict['comment']
        customComment = data_xtb._trans_dict['customComment']
        tp = round( tp_new_value, 2)
        sl = round( sl_new_value, 2)
        # expiration_stamp = data_xtb.timestamp
        response = self.trade_transaction(symbol, mode, trans_type = trans_type,volume = volumen, price=price,order= order_id,
                                          tp=tp, sl=sl,  comment=comment , customComment= customComment ) #expiration = expiration_stamp, customComment=custom_Messege,
        # response = self.trade_transaction(symbol, MODES.SELL_LIMIT.value, 0, volume=volumen_with_dolars, price=price,customComment=custom_Messege, tp=tp, sl=sl)
        status, status_messg = self.manage_response(expiration_stamp, response)
        # if status_messg == 'Invalid prices(limit)':
        #     response = self.trade_transaction(symbol, mode, trans_type=0, volume=volumen, price=price_2,customComment=custom_Messege, tp=tp, sl=sl, expiration=expiration_stamp,comment=custom_Messege )
        ...
        #'Instrument is quoted in close only mode' FAIL 100%      'Instrument is quoted in close only mode'
        if status != 3:
            self.LOGGER.debug(f"FAIL. opening trade of {symbol} Message: {status_messg} of   with {mode} ")
            raise TransactionRejected(str(status) + " Message : "+ status_messg + ' Stock '+symbol+ " Volumne: "+str(volumen))
        else:
            self.LOGGER.debug(f"Successfully. opening trade of {symbol}   with {mode} ")
        return response

    @staticmethod
    def get_tp_sl(mode, price, sl_per, tp_per):
    ...
    @staticmethod
    def change_to_order_type_mode( mode_name):
