...
def get_stock_history_Tech_download(stockId, opion, get_technical_data = False, prepost=True, interval="30m",
    ...
    if df_his is None:
        ...
        if get_technical_data:
            ...
            df_his = get_technical_data_and_HKEX(costum_columns, df_his, interval, opion)
...
def get_technical_data_and_HKEX(costum_columns, df_his, interval, opion):
    print("get_technical_data_and_HKEX", df_his.shape, df_his.columns)
    ...
    if costum_columns is None or any("NQ_" in co for co in costum_columns):
        exter_id_NQ = "^HSI"  # Example for HKEX, change to "^NQ=F" for NASDAQ
        print("get_technical_data_and_HKEX exter_id_NQ", exter_id_NQ, interval, opion)
        df_his = get_external_factor_HKEX(df_his, exter_id_NQ, interval, opion, remove_str_in_colum="^",
                                     startswith_str_in_colum='HSI_')
        print("get_technical_data_and_HKEX df_his", df_his.head(10))
...
def get_external_factor_HKEX(df_his, exter_id_NQ, interval, opion, remove_str_in_colum = "=F",startswith_str_in_colum = 'NQ_'):
    df_ext = get_HKEX_data(exter_id_NQ, interval, opion, remove_str_in_colum)
...
def get_HKEX_data(exter_id_NQ, interval, opion, remove_str_in_colum):
    print("get_HKEX_data", exter_id_NQ, interval, opion)
    ...
    # df_ext['Date'] = pd.to_datetime(df_ext['Date']) - pd.Timedelta(hours=8)
