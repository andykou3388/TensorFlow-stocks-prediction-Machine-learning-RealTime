...
def send_MULTI_alert_and_register(S, df_mul_r):
    ...
    if len(list_models_to_predict_POS) > 0:
        ...
        Logger.logr.info("Score POS: " + str(score_POS) + " for stock: " + S + " with models: " + str(list_models_to_predict_POS))
    ...
    if len(list_models_to_predict_NEG) > 0:
        ...
        if (df_mul["NEG_score"][1:] >= score_NEG).any() and any((df_mul[list_models_to_predict_NEG][1:] >= 91).any()) :# or 1 == 1 :
            Logger.logr.info("Score NEG: " + str(score_NEG) + " for stock: " + S + " with models: " + str(list_models_to_predict_NEG))
            # from XTB_api import xtb_api
...
def send_image_full_alersts(S, df_mul, op_buy_sell : _KEYS_DICT.Op_buy_sell ,  list_models_to_predict :list):
    ...
    path_imgs_tech = ""
    # message_aler = "Stock: <b>" + S + "</b> " + op_buy_sell.value.upper() + "\n"
...
def is_already_sent_alert(df_mul):
    ...
    Logger.logr.debug(f"df_sent: {df_sent[(df_sent['ticker'] == df_mul['ticker'][1:].values[0])].shape[0]}")
    num_is_in_the_sent_list = df_sent[(df_sent['ticker'] == df_mul["ticker"][1:].values[0])].shape[0]
